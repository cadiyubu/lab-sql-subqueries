...
WHERE co.country = ( SELECT co.country FROM sakila.country co WHERE co.country LIKE 'canada');
...
SELECT actor_id, Num_actors_film FROM (
	SELECT fa.actor_id, COUNT(fa.film_id) AS Num_actors_film 
    FROM sakila.film_actor fa
    GROUP BY fa.actor_id) AS actor_counts
GROUP BY actor_id
ORDER BY Num_actors_film DESC
LIMIT 1;

SELECT f.title FROM sakila.actor a
